...
function setupEventListeners() {
  ...
  document.querySelectorAll(".flyer-download-btn").forEach((btn) => {
    btn.addEventListener("click", function (event) {
      const filename = this.getAttribute("data-filename")
      const flyerName = this.getAttribute("data-flyer-name")
      downloadFlyer(filename, flyerName)
    })
  })
}
...
function showNotification(message, type = "info", title = "") {
  ...
  const titleHtml = title
    ? `<div class="notification-content"><i class="fas ${icon}"></i><span>${title}</span></div>`
    : ""

  notification.innerHTML = `
        ${titleHtml}
        <div class="notification-content"><i class="fas ${icon}"></i><span>${message}</span></div>
...
// Flyer download function
function downloadFlyer(filename, flyerName) {
  // Show loading state
  const button = document.querySelector(".flyer-download-btn")
  const originalText = button.innerHTML
  button.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Téléchargement...'
  button.disabled = true

  // Simulate download process with shorter delay
  setTimeout(() => {
    ...
    showNotification(`${flyerName} téléchargé avec succès !`, "success")
    ...
    if (typeof gtag !== "undefined") {
      gtag("event", "download", {
        event_category: "Flyers",
        event_label: flyerName,
        value: 1,
      })
    }
  }, 800) // Reduced delay from 1500ms to 800ms for faster response
}
